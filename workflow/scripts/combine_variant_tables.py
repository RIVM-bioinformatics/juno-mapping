...
def main(args: argparse.Namespace) -> None:
    ...
    if args.mqc is not None:
        ...
        df_wide = (
            pd.pivot(data=df_grouped, index="Sample", columns="FILTER", values="Count")
            .reset_index()
            .fillna(0)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", type=pathlib.Path, required=True, metavar="PATH", nargs="+"
    )
    parser.add_argument("-o", "--output", type=str, required=True, metavar="PATH")
    parser.add_argument(
        "-f", "--fields", type=str, required=True, metavar="FIELD", nargs="+"
    )
    parser.add_argument(
        "--mqc", help="Append data to this MultiQC header", type=pathlib.Path
    )
