rule copy_ref:
    ...
    message: "Copying reference genome to output directory"
    shell:
        """
cp {input} {output}
        """
...
rule bwa_mem:
    ...
    message: "Mapping reads for {wildcards.sample}"
    params:
...
rule sam_to_sorted_bam:
    ...
    message: "Convert sam to sorted bam for {wildcards.sample}"
    conda:
        "../envs/bwa_samtools.yaml"
    container:
        "docker://staphb/samtools:1.17"
...
rule MarkDuplicates:
    ...
    message: "Marking and removing optical duplicates for {wildcards.sample}"
    conda:
        "../envs/gatk_picard.yaml"
    container:
...
rule index_bam:
    ...
    message: "Indexing bam file of {wildcards.sample}"
    conda:
        "../envs/bwa_samtools.yaml"
    container:
        "docker://staphb/samtools:1.17"
